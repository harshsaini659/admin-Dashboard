...
exports.createVariant = async (req, res) => {
    try{
        ...
        if(!Array.isArray(value) || value.length === 0) return res.status(400).json({ message: "Value must be a non-empty array" })
        ...
        const newVariant = new Variant({
            ...
            value: value.map(v => v.trim()) //trim each value in the array
        })
...
exports.editVariant = async (req, res) => {
    try{
        ...
        const updatedVariant = await Variant.findByIdAndUpdate(
            ...
            {
            name: name.trim(),
            value: Array.isArray(value) ? value.map(v => v.trim()) : []
            },
